...
// 使用變數代稱設定路由
router.post('/coach-list', (req, res) => {
  log.info(req.body);
  let Sort = '';
  ...
  const field = req.body.name.slice(0, 4);
  ...
  let langVal = 0;
  if (req.body[9]) langVal += 1; // chinese
  if (req.body[10]) langVal += 2;// eng
  if (req.body[11]) langVal += 4;// jap
  ...
  db.query(`SELECT * FROM coach ${where} ${Sort}`, (error, results) => {
    ...
    output = results.map((el) => {
      if (output.indexOf(el.classSid) === -1) {
        output.push(el.classSid);
        return el;
      }
      return undefined;
    });
...
// 教練資料庫四 - 隨機排序
router.post('/coach-random', (req, res) => {
  log.info(req.body);
